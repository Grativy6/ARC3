...
from .models import CurriculumCase, CurriculumFamily, CurriculumSpec, LevelSpec, Point
...
def derive_seed(index: int) -> int:
    ...
    digest = hashlib.sha256(f"arc3-build003-heldout-v1\\0{index}".encode()).digest()
...
def frozen_seeds() -> tuple[int, ...]:
    ...
    return tuple(derive_seed(index) for index in range(30))


def case_for_seed(seed: int) -> CurriculumCase:
    ...
    digest = hashlib.sha256(f"{PROTOCOL_ID}\0{seed}".encode()).hexdigest()
    return CurriculumCase(case_id=f"b003c-{digest[:16]}", seed=seed)
...
def generate_curriculum(seed: int) -> CurriculumSpec:
    ...
    levels = (
        ...
        _level(
            ...
            restoration_amount=rng.randint(10, 14),
        ),
        _level(
            ...
            restoration_amount=rng.randint(8, 12),
        ),
        _level(
    ...
    return CurriculumSpec(case=case, levels=levels)
