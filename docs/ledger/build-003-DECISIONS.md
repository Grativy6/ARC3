...
- **Official observation:** Campaign 18 executed the planned marker-14 separation at action 41. The returned frame cleared the exact sector overlay, preserved every target-sector color and raw component boundary, and earned the marker-14 target-identity constraint, which remained active through action 54. The repaired bootstrap at action 54 localized active color 0 at `(6,43)` exactly as predicted. The later failure is a distinct bounded-lookahead burden recorded as B-003-0011; level 3 and `NOT_FINISHED` remained authoritative.
